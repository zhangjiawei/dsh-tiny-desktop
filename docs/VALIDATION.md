...
- macOS Intel 隔离原生候选以关闭自动启动的临时设置运行，确认新版设置字段和标准 Edit 菜单可访问；点击应用菜单的“开发者工具”后产生独立 Inspector 原生窗口。候选正常退出且所有临时目录与交叉编译产物已删除，正式 3080 服务保持原 PID。Windows 原生 Inspector 仍由发布工作流验证；Wails beta.16/17 的 Linux `production,devtools` 标签缺少必要实现，因此 Linux 正式包保持 production 与普通右键，不启用全局 debug 模式。
...
- 用户实际从任务执行 `npx @deepseek-ai/dsh plugin ...` 复现 npm 回落 `~/.npm` 并因历史 root-owned 缓存报 `EPERM`。回归确认根因是 DSH 父进程缺少 `npm_config_cache`，并锁定安装器、DSH 与任务统一使用 Tiny `runtime/npm-cache`；未修改用户 `~/.npm` 权限或内容。
- 修复包替换本机候选并认证就绪后，真实 DSH 父进程与 npm 自身均报告 cache 为 Tiny `runtime/npm-cache`；私有 `npx` 成功执行 `@deepseek-ai/dsh@0.1.2-rc.1 --version`。用户 `~/.npm` 文件数前后均为 4265；验证产生的 507 MiB 私有 npm/npx 下载随后清为 0B、0 文件。
...
- 设置模型新增互斥的托管/自定义模式与推荐、稳定、预览、固定版本通道。迁移回归覆盖旧空命令、`dsh web`、历史默认 pnpm dlx、附加私有 trusted-host 和真正的自定义版本；只有历史等价默认进入托管模式，自定义文本保持不变。
- 更新模块回归覆盖 npm 预览/稳定解析、数字预发布排序、仓库不可达、固定版本校验、托管参数越权、运行时指针目录逃逸、旧 receipt 保持当前版本、Profile 控制状态快照/恢复、手动双向回退，以及中断遗留只清理非活动槽位。
- 安装器不再写死原生依赖版本：目标 DSH 以 `--ignore-scripts` 安装后读取实际依赖清单，只允许 subprocess-local、node-pty、koffi 的精确解析版本执行 rebuild；CLI `--version` 自检、六插件固定版本注册与真实认证就绪仍是切换条件。
