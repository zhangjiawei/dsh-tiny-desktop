...
- **DSH 版本与启动命令**：默认是“Tiny 托管”，版本、安装、切换和回退只有一个管理入口。用户只填写附加参数；端口、`web`、`--no-open`、监听地址和认证始终由外壳管理。局域网分享可用 `--trusted-host <私有 IPv4>` 指定网卡，留空则选择默认路由网卡。普通启动只复用已安装版本，不联网、不解析浮动标签，也不静默升级。
