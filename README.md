...
- **谨慎重启**：概览页的“↻ 重启 DSH”、停止服务、应用运行设置、DSH 升级/回退和运行中导入都会先说明任务中断风险并要求确认。Tiny 目前没有 DSH 官方的稳定任务空闲接口，因此不会假装判断 Agent 状态，也不提供隐式的“空闲后重启”。
- **运行环境待激活**：Tiny 以通用方式监听独立 Profile 的依赖清单和锁文件，不识别插件名称、插件市场或私有更新标记。运行中发生任何依赖变化都只显示“重启后生效”，当前 DSH 和任务保持运行；只有用户明确确认重启，或以后正常停止再启动，才激活最终状态。Web 页自己的“重连”仍不等同于 Tiny 管理的进程级重启。
- **新安装默认开关**：登录时启动、登录后隐藏窗口、自动启动 DSH、关闭后继续运行、仅驻留托盘和局域网分享开启；工作空间置顶关闭。隐藏登录启动不显示桌面窗口，仍可从菜单栏/托盘恢复；普通手动启动照常显示。升级保留已有设置，不覆盖用户选择。凭据导入仍需单独主动勾选。
